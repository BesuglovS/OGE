begin
  ...
  begin    
    ...
    var asnw := answ_vars[Random(answ_vars.Length)];
    var answ_num := asnw.Length;
    var data_size := (answ_num + 2) * encoding[1]; 
    var lb := '>';
    
    Write(f, 'Задача №' + i.ToString + lb);
    Write(f, $'В одной из кодировок {encoding[0]} каждый символ кодируется {encoding[1]} битами.'.Replace('  ', ' ') + lb);
    Write(f, $'При подготовке сообщения {name} текст (в нём нет лишних пробелов):' + lb);
    Write(f, $'У нас есть список {task_type}: {answ_vars_str}.' + lb);
    Write(f, $'Затем {del_name} из списка одно из названий, ' + 
        'а также лишнюю запятую и пробел - два пробела не должны идти подряд.' + lb);
    if data_size mod 8 = 0 then
      Write(f, $'При этом размер нового сообщения в данной кодировке ' + 
    ...
    Write(f, $'Напишите в ответе удалённый элемент.');
    Write(f, #9);
    Write(f, 'Ответ' + lb);
    Writeln(f, asnw);
  end;
